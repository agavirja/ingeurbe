...
# streamlit run D:\Dropbox\Empresa\Urbex\Clientes\Ingeurbe\Home.py
...
def main():
    ...
    with st.spinner('Cargando información'):
        ...
        with col1: 
            data_base['nombre'] = data_base['Nombre completo'].apply(lambda x: x.title().strip() if isinstance(x,str) else None)
            options = [x.title() for x in list(sorted(data_base['nombre'].unique())) if isinstance(x,str)]
            options = ['Todos'] + options
            selectname  = st.selectbox('Selección por nombre:',options=options)
            if 'todo' not in selectname.lower():
                data_base = data_base[data_base['nombre']==selectname]
        
        with col2: 
            data_base['identificacion'] = data_base['ID'].apply(lambda x: str(x) if isinstance(x,str) else None)
            options = [x.title() for x in list(sorted(data_base['identificacion'].unique())) if isinstance(x,str)]
            options = ['Todos'] + options
            selectid = st.selectbox('Selección por ID:',options=options)
            if 'todo' not in selectid.lower():
                data_base = data_base[data_base['identificacion']==selectid]
        
        data_activos   = data_activos[(data_activos['Identificacion'].isin(data_base['identificacion'])) | (data_activos['C.C.'].isin(data_base['C.C.']))  | (data_activos['C.C.'].isin(data_base['ID']))]
        data_vehiculos = data_vehiculos[(data_vehiculos['C.C.'].isin(data_base['identificacion'])) | (data_vehiculos['C.C.'].isin(data_base['C.C.']))  | (data_vehiculos['C.C.'].isin(data_base['ID']))]
        data_lasttrans = data_lasttrans[(data_lasttrans['C.C.'].isin(data_base['identificacion'])) | (data_lasttrans['C.C.'].isin(data_base['C.C.']))  | (data_lasttrans['C.C.'].isin(data_base['ID']))]
        data_anotaciones = data_anotaciones[(data_anotaciones['C.C.'].isin(data_base['identificacion'])) | (data_anotaciones['C.C.'].isin(data_base['C.C.']))  | (data_anotaciones['C.C.'].isin(data_base['ID']))]
        ...
        if 'todo' not in selectname.lower() or 'todo' not in selectid.lower():

            if not data_activos.empty:
                st.write('')
                titulo = 'Predios'
                html   = f"""<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Título Centrado</title></head><body><section style="text-align: center;"><h1 style="color: #ABCB38; font-size: 20px; font-family: Arial, sans-serif;font-weight: bold;">{titulo}</h1></section></body></html>"""
                texto  = BeautifulSoup(html, 'html.parser')
                st.markdown(texto, unsafe_allow_html=True)
                    
                variables = ['C.C.', 'Nombre completo', 'Chip', 'Direccion', 'Matricula Inmobiliaria', 'Cedula Catastral', 'copropiedad', 'Avaluo Catastral', 'Impuesto Predial', 'Ano', 'areaconstruida']
                variables = [x for x in variables if x in data_activos]
                datashow  = data_activos[variables]
                datashow.rename(columns={'areaconstruida':'Área construida'},inplace=True)
                st.dataframe(datashow)   
    
            if not data_anotaciones.empty:
                st.write('')
                titulo = 'Transacciones y/o anotaciones'
                html   = f"""<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Título Centrado</title></head><body><section style="text-align: center;"><h1 style="color: #ABCB38; font-size: 20px; font-family: Arial, sans-serif;font-weight: bold;">{titulo}</h1></section></body></html>"""
                texto  = BeautifulSoup(html, 'html.parser')
                st.markdown(texto, unsafe_allow_html=True)
                
                variables = ['C.C.', 'Nombre completo', 'codigo', 'anotacion', 'cuantia', 'tipo_documento_publico', 'numero_documento_publico', 'fecha_documento_publico', 'oficina', 'entidad']
                variables = [x for x in variables if x in data_anotaciones]
                datashow  = data_anotaciones[variables]
                datashow.rename(columns={'areaconstruida':'Área construida'},inplace=True)
                st.dataframe(datashow)   
            ...
            if not data_vehiculos.empty:
                ...
                variables = ['C.C.', 'Nombre completo', 'marca', 'linea', 'modelo', 'carroceria', 'capacidad personas', 'placa', 'porcentaje', 'Tipo de servicio']
                variables = [x for x in variables if x in data_vehiculos]
                datashow  = data_vehiculos[variables]
                datashow.rename(columns={'areaconstruida':'Área construida'},inplace=True)
                st.dataframe(datashow)       
        #st.dataframe(data_base)
        #st.dataframe(data_activos)    
        #st.dataframe(data_vehiculos)
        #st.dataframe(data_lasttrans)
        #st.dataframe(data_anotaciones)
